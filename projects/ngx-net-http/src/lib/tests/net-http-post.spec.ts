...
describe('NetHttpService POST', () => {
  ...
  it('When the post method is called with a response type of ArrayBuffer while providing all callbacks, it should invoke all the callbacks, except the error callback', () => {
    // Arrange
    ...
    const subscription = service.post<ArrayBuffer>({
      body: {
        key: 'value'
      },
      options: {
  ...
  it('When the post method is called with a response type of Blob while providing all callbacks, it should invoke all the callbacks, except the error callback', () => {
    // Arrange
    let onRequestSentCalled = false;
    let onReceivedResponseHeaderCalled = false;
    let onReceivedResponseHeaderResult = false;
    ...
    // Act
    const subscription = service.post<Blob>({
      body: {
        key: 'value'
      },
      options: {
  ...
  it('When the post method is called with a response type of Text while providing all callbacks, it should invoke all the callbacks, except the error callback', () => {
    // Arrange
    let onRequestSentCalled = false;
    let onReceivedResponseHeaderCalled = false;
    let onReceivedResponseHeaderResult = false;
    ...
    // Act
    const subscription = service.post<string>({
      body: {
        key: 'value'
      },
      options: {
  ...
  it('When the post method is called with a response type of Json while providing all callbacks, it should invoke all the callbacks, except the error callback', () => {
    // Arrange
    let onRequestSentCalled = false;
    let onReceivedResponseHeaderCalled = false;
    let onReceivedResponseHeaderResult = false;
    ...
    // Act
    const subscription = service.post<JsonTestResult>({
      body: {
        key: 'value'
      },
      options: {
  ...
  it('When the post method is called without a response type while providing all callbacks, it should select the Json response type and invoke all the callbacks, except the error callback', () => {
    // Arrange
    let onRequestSentCalled = false;
    let onReceivedResponseHeaderCalled = false;
    let onReceivedResponseHeaderResult = false;
  ...
  it('When a file is being uploaded while providing all callbacks, it should invoke all the callbacks, except the error callback', () => {
    // Arrange
    let onRequestSentCalled = false;
    let onReceivedResponseHeaderCalled = false;
    let onReceivedResponseHeaderResult = false;
    ...
    let uploadProgressResult = false;
    let onErrorCalled = false;

    // Act
    const subscription = service.post<JsonTestResult>({
      body: new FormData().append('test', new Blob(['1', '2', '3']))
    }, {
      onRequestSent: () => {
        onRequestSentCalled = true;
      },
      ...
      uploadProgress: (loaded: number, total?: number) => {
        ...
        uploadProgressResult = loaded == 50 && total == 100;
      },
      onError: (error: any) => {
        onErrorCalled = true;
      }
    ...
    expect(request.request.method).toBe('POST');
    request.event(responseHeaderEvent);
    request.event(uploadProgressEvent);
    request.flush({
      key1: 'result',
      key2: 123,
      key3: true,
    ...
    expect(onReceivedBodyResult).withContext('onReceivedBody has correct parameter').toBeTrue();
    expect(onRequestCompletedCalled).withContext('onRequestCompleted is called').toBeTrue();
    expect(uploadProgressCalled).withContext('uploadProgress is called').toBeTrue();
    expect(uploadProgressResult).withContext('uploadProgress has correct parameter').toBeTrue();
    expect(onErrorCalled).withContext('onError is not called').toBeFalse();
  });
});
